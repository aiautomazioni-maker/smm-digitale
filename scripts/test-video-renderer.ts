...
import { renderVideoFromEDL } from '../src/lib/video-renderer';
...
async function testEDLRender() {
    ...
    try {
        ...
        const outputBuffer = await renderVideoFromEDL(videoBuffer, mockEDL as any);

        const outPath = path.join(process.cwd(), 'edl-test-output.mp4');
